...
export class PrimenguiFormComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    this.isGroup =
      this.controls && this.controls.length > 0 && this.controls[0].controls
        ? true
        : false;
    this.setClassMap();
    this.setFormGroup();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      ...
      changes.controls.previousValue &&
      changes.controls.currentValue !== changes.controls.previousValue
    ) {
      this.setFormGroup();
    }
  }
  ...
  setClassMap() {
    this.classMap = {
      [`primengui-form-group`]: this.isGroup,
      [`primengui-form-${this.theme}`]: true,
    };
  }
